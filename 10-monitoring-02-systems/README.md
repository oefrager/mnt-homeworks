...
выведите в мониторинг и почему?
   ...
   К самому минимуму для данного ТЗ можно отнести:
    - CPU LA  - т.к. указали на высокозагруженные вычисления.
    - RAM     - т.к. нехватка может привести к потере выходных данных 
    - inodes  - т.к. отчеты используют диск.
    - Net     - т.к. взаимодействме происходитт по сети.
...
Некоторые минусы pull-модели:

    Возможны пропуски данных. Это происходит, если клиент не проверяет сервер на наличие новых сообщений слишком долго.  1
    Увеличенная нагрузка на сервер. Она возникает из-за повторных запросов со стороны множества клиентов, особенно если данные обновляются часто.  1
    Привязка к определённым инструментам. Pull-подход ограничивает возможность настройки процесса развёртывания, так как нужно использовать инструменты, оперирующие pull. 2
#Плюсы push-модели:
упрощение репликации данных в разные системы мониторинга или их резервные копии
более гибкая настройка отправки пакетов данных с метриками
UDP — это менее затратный способ пеередачи данных, из-за чего может возрасти
